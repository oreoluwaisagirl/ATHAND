  ...
  return (
    <div className="min-h-screen bg-background pb-24 md:pb-10">
      <section className={`relative overflow-hidden bg-gradient-to-r ${accent}`}>
        <div className="absolute left-0 top-0 h-56 w-56 rounded-full bg-white/40 blur-3xl" />
        <div className="absolute bottom-0 right-0 h-64 w-64 rounded-full bg-[#ffd7bf]/40 blur-3xl" />
        <div className="relative mx-auto max-w-7xl px-4 pb-12 pt-6 sm:px-8 lg:px-10 lg:pb-16 lg:pt-10">
          <button
            type="button"
            onClick={handleBack}
            className="inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/70 px-4 py-2 text-sm font-medium text-text-primary shadow-sm transition hover:bg-white"
          >
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
            <div className="max-w-3xl">
              ...
              {badge ? (
                <div className="mt-6 inline-flex rounded-full bg-white px-4 py-2 text-sm font-semibold text-accent shadow-sm">
                  {badge}
                </div>
              ) : null}
            </div>
            ...
            {aside ? (
              <div className="rounded-[1.8rem] border border-white/60 bg-white/80 p-5 shadow-[0_18px_40px_rgba(39,55,86,0.08)] backdrop-blur">
                {aside}
              </div>
            ) : null}
          </div>
